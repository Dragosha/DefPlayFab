...
dirtylarry.action_id_enter       = hash("enter")
...
function dirtylarry.input(self, node, action_id, action, type, empty_text, enter_cb)
    ...
    if (dirtylarry.active_node == input_node) then
        ...
        -- input enter
        elseif (action_id == dirtylarry.action_id_enter and (action.released)) then
            if enter_cb then enter_cb(self, input_node.data) end
            gui.hide_keyboard()
            input_node.active = false
            dirtylarry.active_node=nil
        -- input deletion
...
function dirtylarry.scrollarea(self, node_str, action_id, action, scroll, cb)
	...
	if cb and  not consumed_input and ((touch and hit) or not touch) then
